...
> **Verified in this environment on 2026-08-02:** 365 passed, 0 failed, on macOS (Darwin 24.6.0) with Python 3.14.3. Skips are environment-dependent (browser tests without the optional Playwright extra, and live-network tests when a source is unreachable). Bandit static analysis reports 0 high and 0 medium findings. Under a strict evidence rubric, 112 of 167 catalogued capabilities are PRODUCTION READY, each backed by a real acceptance test that exercises the feature and passes on a clean clone. 9 are blocked by an external platform with a precise reason, and the remaining 46 are still being finished on real evidence. D-Eye as a whole is not yet claimed as production ready or as 100 percent complete.
...
- **Feature catalogue:** under a strict evidence rubric, 112 of 167 catalogued capabilities are PRODUCTION READY, each backed by a real acceptance test that exercises the feature and passes on a clean clone. 9 are blocked by an external platform (a login or anti-bot wall, or a vendor that exposes no public automation API), each with a precise recorded reason. The remaining 46 are still being finished on real evidence; none is inflated.
- **Not claimed:** D-Eye as a whole is not production ready, and no claim of 100 percent completion is made.

Honest roadmap:

- A live hosted remote MCP endpoint (the transport and a Docker compose scaffold ship today; no hosted endpoint has been stood up).
- Cross operating system installer verification on Linux and Windows runners.
- Signed release bundles with reproducible build verification.
- The optional local browser edge (Playwright), moved from opt in extra to a verified integration test.
- An optional local semantic search adapter (embeddings plus a local vector index) behind a replaceable interface.
