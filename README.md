...
> **Verified on 2026-08-02:** the full suite passes with 0 failures both in this environment (macOS, Darwin 24.6.0, Python 3.14.3; 417 passed) and on a fresh public clone via `scripts/clean_clone_gate.sh`, which installs the documented extras plus the optional `.[browser]` extra and `playwright install chromium` and runs everything (414 passed, 0 failed). Skips are environment-dependent (structural browser tests when Playwright is present; live-network tests when a source is unreachable or rate-limited). Bandit static analysis reports 0 high and 0 medium findings. Under a strict evidence rubric, 139 of 167 catalogued capabilities are PRODUCTION READY, each backed by a real acceptance test that exercises the feature and passes on a clean clone. 9 are blocked by an external platform with a precise reason, and the remaining 19 are still being finished on real evidence. D-Eye as a whole is not yet claimed as production ready or as 100 percent complete.
...
- **Feature catalogue:** under a strict evidence rubric, 139 of 167 catalogued capabilities are PRODUCTION READY, each backed by a real acceptance test that exercises the feature and passes on a clean clone. 9 are blocked by an external platform (a login or anti-bot wall, or a vendor that exposes no public automation API), each with a precise recorded reason. The remaining 19 are still being finished on real evidence (18 implemented but not fully verified, 1 partial); none is inflated.
