...
D-Eye's own modules, tests, and CLI/plugin surface. Row-level status
per feature ID lives in [`../assets/../docs/FEATURE_TRACEABILITY.csv`](../../docs/FEATURE_TRACEABILITY.csv)
(regenerated by `scripts/build_traceability.py`)._
...
verify_repo,sign_release,scan_licences}.py, scripts/install.sh`. Tests:
...
- `scripts/{scan_licences,run_pip_audit,sign_release,gen_sbom}.py` +
  `docs/DEPENDENCY_AND_LICENCE_POLICY.md, docs/SIGNED_RELEASE.md`.
...
live Playwright, cross-platform runners, hosted remote MCP, Claude
cross-surface activation — all documented with exact reasons in
